...
function dataHandler(res){
	...
	externalInterface.writeDevice({data: data})
...
function write_radio_device(args){
	if(port){
		port.write(`B${args.data}`, function(err){
			if(err){
...
function get_radio_parameters(args){
	return {tag: radioTag, tx: dataTXCounter, rx: dataRXCounter, type: radioType}
}
